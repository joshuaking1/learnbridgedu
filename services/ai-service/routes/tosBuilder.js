...
router.post("/", async (req, res) => {
  ...
  if (!subject || !classLevel || topics.length === 0) {
    return res.status(400).json({
      error:
        "Missing required fields: subject, classLevel, and at least one topic are required",
    });
  }
...
`;
    ...
    // Return the response with the field name that the frontend expects
    res.status(200).json({
      tableOfSpecification: tosContent, // This is the field name the frontend expects
      tosContent, // Keep this for backward compatibility
      metadata: {
        subject,
        classLevel,
        topics,
    ...
    res
      .status(500)
      .json({ error: "Failed to generate Table of Specifications" });
  }
});
