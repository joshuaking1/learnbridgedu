...
router.post("/", async (req, res) => {
  ...
  if (!subject || !classLevel || !assignmentType) {
    return res.status(400).json({
      error:
        "Missing required fields: subject, classLevel, and assignmentType are required",
    });
  }
...
`;
    ...
    // Return the response
    res.status(200).json({
      rubric: rubricContent, // This is the field name the frontend expects
      rubricContent, // Keep this for backward compatibility
      metadata: {
        subject,
        classLevel,
        assignmentType,
