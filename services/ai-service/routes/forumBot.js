...
// TEMPORARY: Authentication bypassed
router.post("/forum-post", async (req, res) => {
  ...
  try {
    ...
    if (!postId) {
      return res
        .status(400)
        .json({ error: "Missing required postId parameter" });
    }

    // Fetch the post data from the discussion service
    const discussionServiceUrl =
      process.env.DISCUSSION_SERVICE_URL || "http://localhost:3007";
    const fetch = require("node-fetch");

    const postResponse = await fetch(
      `${discussionServiceUrl}/api/posts/${postId}`,
      {
        headers: {
    ...
    if (!postResponse.ok) {
      return res
        .status(404)
        .json({ error: "Post not found or could not be retrieved" });
    }
    ...
    const threadResponse = await fetch(
      `${discussionServiceUrl}/api/threads/${post.thread_id}`,
      {
        headers: {
          Authorization: req.headers.authorization,
        },
    ...
    if (!threadResponse.ok) {
      return res
        .status(404)
        .json({ error: "Thread not found or could not be retrieved" });
    }
    ...
    if (promptEmbedding && promptEmbedding.length === 384 && db) {
      ...
      const vectorSearchQuery = {
        text:
          "SELECT chunk_id, book, subject, class_level, strand, sub_strand, chunk_text, metadata, " +
      ...
      try {
        ...
        if (sbcResults.rows && sbcResults.rows.length > 0) {
          ...
          sbcContextText = sbcResults.rows
            .map((row) => {
              return `[SBC Content - ${row.subject} ${row.class_level} - ${
                row.strand
              }${row.sub_strand ? " / " + row.sub_strand : ""}]\n${
                row.chunk_text
              }`;
            })
            .join("\n\n");
...
${
  sbcResultsFound
    ? "SBC CONTEXT INFORMATION:\n" + sbcContextText
    : "No specific SBC context is available for this query. Provide general educational guidance based on your knowledge."
}
...
// TEMPORARY: Authentication bypassed
router.post("/concept-summary", async (req, res) => {
  if (!groq) {
  ...
  try {
    ...
    if (!context || !context.title || !context.content) {
      return res
        .status(400)
        .json({ error: "Missing required context parameters" });
    }
...
The summary should be approximately 150-250 words.`;
    ...
    const conceptsPrompt = `Based on this thread: "${
      context.title
    }" with content: "${context.content.substring(
      0,
      1000
    )}...", list only 3-7 key educational concepts as a JSON array of strings. Return only the JSON array, nothing else.`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You extract key educational concepts from text and return them as a JSON array of strings. Respond with only the JSON array, no other text.",
        },
      ...
      resources = [
        {
          ...
          description:
            "Our AI encountered an error generating specific resources. Please try again later.",
          url: "#",
          type: "Error",
        },
      ];
