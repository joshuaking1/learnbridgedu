  ...
  try {
    ...
    if (process.env.JWT_SECRET) {
      ...
      const tokenHS256 = jwt.sign(
        { userId: "test-user-123", role: "teacher" },
      ...
      try {
        console.log(`Sending request to ${baseUrl}/api/ai/limits with token`);
        const response = await axios.get(`${baseUrl}/api/ai/limits`, {
          headers: {
        ...
        console.error(
          "Authentication failed with HS256 token:",
          authError.message
        );
      ...
      const tokenHS512 = jwt.sign(
        { userId: "test-user-456", role: "teacher" },
        process.env.JWT_SECRET,
        {
          expiresIn: "1h",
          algorithm: "HS512",
      ...
      try {
        ...
        console.error(
          "Authentication failed with HS512 token:",
          authError.message
        );
        if (authError.response) {
          console.error("Response:", authError.response.data);
      ...
      const tokenExpired = jwt.sign(
        { userId: "test-user-789", role: "teacher" },
        process.env.JWT_SECRET,
        {
      ...
      try {
        ...
        console.log(
          "Authentication successful with expired token (should only happen if IGNORE_TOKEN_EXPIRATION=true):"
        );
        console.log(JSON.stringify(response.data, null, 2));
      } catch (authError) {
        console.error(
